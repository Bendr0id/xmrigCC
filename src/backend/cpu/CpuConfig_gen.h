...
template<>
size_t inline generate<Algorithm::CN_EXTREMELITE>(Threads<CpuThreads> &threads, uint32_t limit)
{
  return generate("cn-extremelite", threads, Algorithm::CN_EXTREMELITE_0, limit);
}
#endif
...
size_t inline generate<Algorithm::RANDOM_X>(Threads<CpuThreads> &threads, uint32_t limit)
{
    ...
    count += generate("rx/keva", threads, Algorithm::RX_0, limit);
